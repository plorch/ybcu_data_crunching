...
dsn21a = "C:/Users/PatrickLorch/SSRS/Southern Sierra Research Station - Documents/Projects/LCR YBCU/Data/2021/Data/Shapefiles edited to include C2748"
lcr_ybcu_survey_points_spdf_2021a = readOGR(dsn = dsn21a, layer = "SurveyPoint2021")
proj4string(lcr_ybcu_survey_points_spdf_2021a)
sp::coordinates(lcr_ybcu_survey_points_spdf_2021a)
lcr_ybcu_detections_spdf_2021a = readOGR(dsn = dsn21a, layer = "Detection2021")
proj4string(lcr_ybcu_detections_spdf_2021a)
sp::coordinates(lcr_ybcu_detections_spdf_2021a)
...
proj4string(lcr_ybcu_survey_points_spdf_2019)
...
writeOGR(lcr_ybcu_survey_points_spdf_UTM11_2021,
         ...
         driver = "CSV")
writeOGR(lcr_ybcu_survey_points_spdf_UTM11_2021a,
         dsn = "stuff2a",
         layer = "survey_points_2021a_UTM11N",
         driver = "CSV")
writeOGR(lcr_ybcu_detections_spdf_UTM11_2021a,
         dsn = "stuff2b",
         layer = "detections_2021a_UTM11N",
         driver = "CSV")
...
## Import survey point data for all years
...
points21 = dbReadTable(connec, "points2021a")
...
detections21 = dbReadTable(connec, "detections2021a")
...
points20_wfixes = points20 %>% 
  slice(which(objectid != "110")) %>%
  mutate(survey_key = if_else(objectid == "857",
                              "FY20-YBC-C2365", survey_key))
...
svdet21 = detections21 %>%
  filter(visit_type == "Sur", new_bird == "Yes") %>%
  select(point_key, survey_per, ybcu_num) %>%
  arrange(survey_per, point_key, ybcu_num) 
pt21_det_count = svdet21 %>%
  count(point_key, name = "det_count")

pt21 = points21 %>%
  slice(which(survey_per != "0")) %>%
  select("survey_key", "point_key", "survey_per", "point_num", "cicada_cnt", 
         "play_cnt", "ybcu_detec", "notes", "date_coll", 
         "x", "y") %>%
  rename(svperiod = "survey_per", 
  ...
  left_join(select(cicada12, cicada_index = cicada_code, cicada_cat),
                                  by = c("cicada_cnt" = "cicada_cat")) %>%
  left_join(pt21_det_count, by = c("point_key" = "point_key")) %>%
  mutate(det_count = replace_na(det_count, 0))
pt21[c(197, 299, 303),]
...
pt22 = points22_wfixes %>%
  select("Survey.key", "Point.key", "Survey.period", "Point.number",
         "Cicada.count", "Play.count", "YBCU.detected.", "notes",
         "Date.Collected", "x11e", "y11n") %>%
  rename(svperiod = "Survey.period", 
         numberofplaybacks = "Play.count", 
         ybcudetection = "YBCU.detected.", 
         svpointtime = "Date.Collected",
...
pt19_sum_site = pt19 %>%
  ...
  summarise(points_surveyed = n(),
            ...
            survey_duration_approx = as.numeric(
              max(as.POSIXct(svpointtime, format = "%m/%d/%Y, %I:%M %p")) - 
