...
Reconciling MSCP numbers (surveyor designations) with sites of known age: 

Cari Lynn:  Seems like the data from 2019-2022 can be combined to match many of the sites from the 2018 model (C2529 + C2530 = CVCA03, C2364 + C2365 + C2366 = PVER05; etc.). CVCA 09, North 160, and Middle Hippy Fire were not in the 2018 model but could potentially be added in. 

Cari Lynn will get me a list to crosswalk the sites. This site crosswalk will be used in ybcu_detection.Rmd first.
...
table(pt19$loccode1922, pt19$cicada_index)

pt19_cicada_ind = pt19 %>%
  select(loccode1922, cicada_index) %>%
  group_by(loccode1922) %>%
  summarise(mean_cic_ind = mean(cicada_index))

pt19_cicada_ind_scaled = pt19_cicada_ind %>%
  tibble::column_to_rownames("loccode1922") %>%
  scale()
...
table(pt20$loccode1922, pt20$cicada_index)
...
pt20_cicada_ind_scaled = pt20_cicada_ind %>%
  tibble::column_to_rownames("loccode1922") %>%
  scale()
...
Before import into postgis, points21 must have gotten opened in excel and Cicada.count got converted to dates. This had to be corrected.
...
points21_fixed = points21%>%
  ...
  mutate(Cicada.count = if_else(Cicada.count == "19-Nov",
                                "11-19", 
                                Cicada.count)) %>%
  mutate(Cicada.count = if_else(Cicada.count == "10-Jun",
                                "6-10", 
                                Cicada.count))
...
table(pt21$loccode1922, pt21$cicada_index)
...
pt21_cicada_ind_scaled = pt21_cicada_ind %>%
  tibble::column_to_rownames("loccode1922") %>%
  scale()
...
table(pt22$loccode1922, pt22$cicada_index)

pt22_cicada_ind = pt22 %>%
  select(loccode1922, cicada_index) %>%
  group_by(loccode1922) %>%
  summarise(mean_cic_ind = mean(cicada_index))

pt22_cicada_ind_scaled = pt22_cicada_ind %>%
  tibble::column_to_rownames("loccode1922") %>%
  scale()
...
write.csv(pt19_sbsp, "pt19_sbsp.csv", row.names = F)
write.csv(pt20_sbsp, "pt20_sbsp.csv", row.names = F)
write.csv(pt21_sbsp, "pt21_sbsp.csv", row.names = F)
write.csv(pt22_sbsp, "pt22_sbsp.csv", row.names = F)
...
#### Cicadas summarized by site

```{r cicadas}
pt19_cicada_ind_scaled.df = pt19_cicada_ind_scaled %>%
  as.data.frame(loccode1922 = rownames(.), .) %>%
  tibble::rownames_to_column()
names(pt19_cicada_ind_scaled.df)[1] = "loccode1922"

pt20_cicada_ind_scaled.df = pt20_cicada_ind_scaled %>%
  as.data.frame(loccode1922 = rownames(.), .) %>%
  tibble::rownames_to_column()
names(pt20_cicada_ind_scaled.df)[1] = "loccode1922"

pt21_cicada_ind_scaled.df = pt21_cicada_ind_scaled %>%
  as.data.frame(loccode1922 = rownames(.), .) %>%
  tibble::rownames_to_column()
names(pt21_cicada_ind_scaled.df)[1] = "loccode1922"

pt22_cicada_ind_scaled.df = pt22_cicada_ind_scaled %>%
  as.data.frame(loccode1922 = rownames(.), .) %>%
  tibble::rownames_to_column()
names(pt22_cicada_ind_scaled.df)[1] = "loccode1922"

cicada1922_site_list = 
  list(pt19_cicada_ind_scaled.df,
       pt20_cicada_ind_scaled.df,
       pt21_cicada_ind_scaled.df,
       pt22_cicada_ind_scaled.df)
...
(cicadanames1922 =
  names(cicada1922_site_lj)[-1] %>%
  as_data_frame() %>%
  separate(value, c("Year", NA, NA), sep = "\\.") %>%
  mutate(Year2 = paste0(Year, c(".2019", ".2020", ".2021", ".2022")))
)

names(cicada1922_site_lj)[-1] = 
    as.character(cicadanames1922$Year2)
```
...
### Model with time measure of effort and Cicada for observation
...
# remove loccode1922
sc = cicada1922_site_lj
sc[is.na(sc)] = 0
umf_eff_cic = unmarkedMultFrame(y = presabs1922_sitesbyperiod[,-1],
                 numPrimary = 4,
                 obsCovs = list(effort = 
                                  effort1922_sitebysvperiod_lj[,-1]),
                 # Using more than one row so this is a data.frame not a 
                 # vector. Only first is used.
                 siteCovs = sc[, 2:5],
                 yearlySiteCovs = list(cicada_time =
                                         sc[, c(2:5)])
)

summary(umf_eff_cic)

dynamic_occ_m2 = colext(psiformula = ~mean_cic_ind.2019,
                     gammaformula = ~cicada_time,
                     epsilonformula = ~cicada_time,
                     pformula = ~effort,
                     data = umf_eff_cic,
                     # optim method, leave as "BFGS"
                     method = "BFGS")
...
# Likely to take a Very Long Time on large data sets
# with nsim = 10, takes my Mackbook ~ 60 seconds
mb.boot2 <- AICcmodavg::mb.gof.test(dynamic_occ_m2,
                                   nsim = 1000) # Must be much higher than five to be useful

print(mb.boot2, digit.vals = 4, digits.chisq = 4)

summary(dynamic_occ_m2)

# The predicted occupancy probability for each site during each primary survey (year). This is stored in an array within the model object (dynamic_occ_m1@smoothed). The array is 2 (a vector of probabilities for 1. unoccupied and 2. occupied) x 5 (number of primary surveys) x 100 (number of sites).

# Occupancy probability for each of 4 years by site

occu_by_site2 = as.data.frame.matrix(dynamic_occ_m2@smoothed[2,,])
names(occu_by_site2) = names_count1922$loccode1922
View(occu_by_site2)
...
# Calculate SE for derived occupancy predictions using bootstrap, larger B requires longer time
m2 <- nonparboot(dynamic_occ_m2, 
                 B = 1000)

# Predicted occupancy in each year (with SE)
# the "[2,]" calls the occupied estimates,
# "[1,]" for unoccupied estimates
predicted_occupancy2 <- data.frame(year = c(2019:2022),
                             smoothed_occ = smoothed(dynamic_occ_m2)[2,],
                             SE = m2@smoothed.mean.bsse[2,])
predocc_plot2 = predicted_occupancy2 %>%
  ggplot(aes(x = year, y = smoothed_occ)) +
  geom_line() +
  geom_point() +
  geom_errorbar(aes(ymin = smoothed_occ - SE, 
                ...
                width = 0.2)
print(predocc_plot2)
predocc_plot2 + labs(title="Occupancy Probability with Cicadas, 2019-2022", 
                    x="Year", 
                    y = "Occupancy Probability")+
   theme_classic() +
   scale_color_manual(values='#E69F00')
