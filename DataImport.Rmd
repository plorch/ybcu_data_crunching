...
table(pt22$loccode1922, pt22$cicada_index, pt22$svperiod)
...
pt22_cicada_ind_byperiod = pt22_cicada_ind %>%
  pivot_wider(names_from = svperiod,
              values_from = mean_cic_ind) 

pt22_cicada_ind_byperiod_scaled = pt22_cicada_ind_byperiod %>%
  tibble::column_to_rownames("loccode1922") %>%
  scale()
```
...
```{r cicadas}
pt19_cicada_ind_byperiod_scaled.df = pt19_cicada_ind_byperiod_scaled %>%
  as.data.frame(loccode1922 = rownames(.), .) %>%
  tibble::rownames_to_column()
names(pt19_cicada_ind_byperiod_scaled.df)[1] = "loccode1922"

pt20_cicada_ind_byperiod_scaled.df = pt20_cicada_ind_byperiod_scaled %>%
  as.data.frame(loccode1922 = rownames(.), .) %>%
  tibble::rownames_to_column()
names(pt20_cicada_ind_byperiod_scaled.df)[1] = "loccode1922"

pt21_cicada_ind_byperiod_scaled.df = pt21_cicada_ind_byperiod_scaled %>%
  as.data.frame(loccode1922 = rownames(.), .) %>%
  tibble::rownames_to_column()
names(pt21_cicada_ind_byperiod_scaled.df)[1] = "loccode1922"

pt22_cicada_ind_byperiod_scaled.df = pt22_cicada_ind_byperiod_scaled %>%
  as.data.frame(loccode1922 = rownames(.), .) %>%
  tibble::rownames_to_column()
names(pt22_cicada_ind_byperiod_scaled.df)[1] = "loccode1922"

cicada1922_sitebysvperiod_list = 
  list(pt19_cicada_ind_byperiod_scaled.df,
       pt20_cicada_ind_byperiod_scaled.df,
       pt21_cicada_ind_byperiod_scaled.df,
       pt22_cicada_ind_byperiod_scaled.df)

(cicada1922_sitebysvperiod_lj =  
cicada1922_sitebysvperiod_list %>%
  Reduce(function(df1, df2)
    left_join(df1, df2, by = "loccode1922"), .))

(cicadaperiodnames1922 =
  names(cicada1922_sitebysvperiod_lj)[-1] %>%
  as_data_frame() %>%
  separate(value, c("Period", NA, NA)) %>%
  mutate(Period2 = paste0(Period, 
                          c(rep(".2019", 4),
...
names(cicada1922_sitebysvperiod_lj)[-1] = 
    as.character(cicadaperiodnames1922$Period2)
```
...
umf_eff_cic = unmarkedMultFrame(y = presabs1922_sitesbyperiod[,-1],
                 ...
                 obsCovs = list(effort = 
                                  effort1922_sitebysvperiod_lj[,-1],
                                cicadas = 
                                  cicada1922_sitebysvperiod_lj[,-1])
)

summary(umf_eff_cic)

dynamic_occ_m2 = colext( psiformula = ~cicadas,
                     gammaformula = ~1,
                     epsilonformula = ~1,
                     pformula = ~effort,
                     data = umf_eff_cic,
